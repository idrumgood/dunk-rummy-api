...
import { generateGameSummary } from './utils/aiSummary';
import { User, GameSettings, HandResult, FinalGameResult, StoredGame } from './types/gameTypes';
import { Storage } from '@google-cloud/storage'; // Import the Storage class
...
async function uploadJsonToGCS(data: any, filename: string): Promise<void> {
...
async function downloadJsonFromGCS<T>(filename: string): Promise<T[]> {
    try {
        const file = bucket.file(filename);
...
async function initializeData() {
    users = await downloadJsonFromGCS<User>(usersFileName);
    games = await downloadJsonFromGCS<StoredGame>(gamesFileName);
...
app.post('/games', async (req, res) => {
    const gameData = req.body as Omit<StoredGame, 'id' | 'date'>;
    ...
    newGame.aiSummary = await generateGameSummary({
        p1Name,
        p2Name,
